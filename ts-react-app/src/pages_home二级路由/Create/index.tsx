...
import { updateFollowingCount } from '../../store/slices/userSlice';
import './index.scss';
...
export default function Create() {
  ...
  const loadPersistedData = () => {
    try {
      // 加载关注状态
      const savedFollowedUps = localStorage.getItem('followedUps');
      const followedSet = savedFollowedUps ? new Set<number>(JSON.parse(savedFollowedUps)) : new Set<number>();
      setFollowedUps(followedSet);

      // 加载访问历史
      const savedRecentVisited = localStorage.getItem('recentVisited');
      const visitedList = savedRecentVisited ? JSON.parse(savedRecentVisited) : [];
  ...
  useEffect(() => {
    if (isDataLoaded) {
      try {
        localStorage.setItem('followedUps', JSON.stringify([...followedUps]));
      } catch (error) {
